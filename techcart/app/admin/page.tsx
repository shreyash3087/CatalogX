...
declare global {
  interface Window {
    google?: any;
    __gsiAdminInitTechCart?: boolean;
  }
}
...
export default function TechCartAdminPage() {
  ...
  // 1. Fetch Live Metrics & Orders from techcart_db
  const fetchData = async () => {
    try {
      ...
      console.error('Failed to fetch TechCart admin metrics:', e);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const initGsi = () => {
      ...
      if (!window.__gsiAdminInitTechCart) {
          ...
          callback: (response: any) => {
            if (response?.credential) {
              ...
              if (data) {
                ...
                setUser(u);
                localStorage.setItem('techcart_admin_user', JSON.stringify(u));
              }
            }
          },
          auto_select: false,
        ...
        window.__gsiAdminInitTechCart = true;
      }
    ...
    const stored = localStorage.getItem('techcart_admin_user');
    if (stored) {
      try {
  ...
  const handleSignOut = () => {
    setUser(null);
    localStorage.removeItem('techcart_admin_user');
    if (typeof window !== 'undefined' && window.google?.accounts?.id) {
      try {
  ...
  return (
    <div className="min-h-screen bg-[#06080F] text-slate-100 flex flex-col font-sans">
      {/* Top Admin Header */}
      <header className="bg-[#0A1020] border-b border-cyan-950/80 px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-cyan-500 flex items-center justify-center text-black font-extrabold text-sm">
              ⚡
            </div>
            <span className="text-base font-black text-white tracking-tight">TechCart Admin</span>
          </Link>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        {!user?.isLoggedIn ? (
          <div className="max-w-md mx-auto my-16 p-8 rounded-3xl bg-[#0A1020] border border-cyan-950 text-center space-y-6 shadow-2xl">
            <div className="w-16 h-16 rounded-2xl bg-cyan-500/20 border border-cyan-500 text-cyan-400 text-2xl flex items-center justify-center mx-auto">
              🔒
            </div>
            <div>
              <h2 className="text-xl font-bold text-white">Merchant Admin Authentication</h2>
              <p className="text-xs text-slate-400 mt-1.5 leading-relaxed">
                Sign in with your authorized Google Account to access TechCart Electronics metrics, live orders ledger, and stock controls.
              </p>
            </div>
          ...
          <div className="max-w-md mx-auto my-16 p-8 rounded-3xl bg-rose-950/20 border border-rose-900/50 text-center space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-slate-300">
              You are signed in as <strong className="text-white font-mono">{user.email}</strong>, but this account is not authorized as a Root Admin for TechCart Electronics.
            </p>
            <button
              onClick={handleSignOut}
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-5 rounded-2xl bg-[#0A1020] border border-cyan-950 shadow-sm space-y-1">
                <div className="text-[11px] font-bold uppercase text-cyan-400 font-mono">Agent Discoveries</div>
              ...
              <div className="p-5 rounded-2xl bg-[#0A1020] border border-cyan-950 shadow-sm space-y-1">
                ...
                <div className="text-[11px] text-slate-400">Hardware vector search queries</div>
              </div>
            </div>
            ...
            {activeTab === 'overview' || activeTab === 'orders' ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider font-mono">
                    Autonomous Orders Ledger (techcart_db.orders)
                  </h3>
                  <button
                    onClick={fetchData}
                ...
                <div className="bg-[#0A1020] border border-cyan-950 rounded-2xl overflow-hidden shadow-xl">
                  {orders.length === 0 ? (
                    <div className="py-16 text-center text-slate-500 space-y-2">
                      <div className="text-3xl">📦</div>
                      <div className="text-xs font-bold">No orders recorded in techcart_db yet</div>
                      <p className="text-[11px] text-slate-600 max-w-sm mx-auto">
                        Ask the CatalogX buyer agent on Port 3000 to buy earbuds or keyboards, and the order will appear here in real time.
                      </p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className="bg-[#060A14] text-[10.5px] uppercase font-bold text-slate-400 border-b border-cyan-950">
                          ...
                          {orders.map((o) => (
                            <tr key={o.orderId} className="hover:bg-slate-900/40 transition-colors">
                              ...
                              <td className="py-3.5 px-4">
                                ...
                                <div className="text-[10.5px] text-slate-400 font-mono">{o.brand}</div>
                              </td>
                              <td className="py-3.5 px-4">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider font-mono">
                  Hardware Inventory & Stock (techcart_db.inventory)
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {products.map((p) => (
                    <div
                      key={p.id}
                      className="p-5 rounded-2xl bg-[#0A1020] border border-cyan-950 flex items-center justify-between gap-4"
                    >
                      <div className="min-w-0">
                        <div className="text-[10px] text-cyan-400 uppercase font-mono font-bold">{p.brand}</div>
